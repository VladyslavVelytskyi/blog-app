            ...
            <div class="d-flex justify-content-between align-items-center">
              <div class="btn-group"
                ...
                <router-link :to="{name: 'Post', params: {id: post._id}}"
                             class="btn btn-sm btn-outline-secondary"> View Post </router-link>
                <router-link :to="{name: 'Edit', params: {id: post._id}}"
                             class="btn btn-sm btn-outline-secondary"> Edit Post </router-link>
                <button v-on:click="deletePost(post._id)"
                        class="btn btn-sm btn-outline-secondary"> Delete Post </button>
              </div>
            </div>
            <div class="card-footer">
              <small class="text-mutted">Posted on: {{ post.date_posted }}</small>
              <br>
              <small class="text-mutted">by: {{ post.author }}</small>
            </div>
          </div>
        </div>
      </div>
